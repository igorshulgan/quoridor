...
public class MiniMaxAlphaBeta extends MiniMax {
    ...
    public void move(Map map, int playerNumber) throws Exception {
        moveOneStep(map, playerNumber, depth, playerNumber, MIN, MAX, MAX);
    }

    public static int checkMove(Map map, int playerNumber, int newX, int newY, int depth, int playerNumberOriginal, int alpha, int beta, int maxMoveBorder) throws Exception {
        ...
        if (depth > 0) {
            move = moveOneStep(map, playerNumber ^ 1, depth - 1, playerNumberOriginal, alpha, beta, maxMoveBorder);
        }
    ...
    public static int checkStickPlacement(Map map, int playerNumber, int x1, int y1, int x2, int y2, int depth, int playerNumberOriginal, int alpha, int beta, int maxMoveBorder) throws Exception {
        Player player = map.getPlayer(playerNumber);
        Player otherPlayer = map.getPlayer(playerNumber ^ 1);
    ...
    public static Move moveOneStep(Map map, int playerNumber, int depth, int playerNumberOriginal, int alpha, int beta, int maxMoveBorder) throws Exception  {
        int maxValue = MIN;
        Move move = null;
        Player player = map.getPlayer(playerNumber);
        Player otherPlayer = map.getPlayer(playerNumber ^ 1);
                ...
                for (int j = 0; j < 17; j++) {
                    for (int i1 = 0; i1 < movesX.length; i1++) {
                        ...
                        if (Stick.check(x1, y1) && Stick.check(x2, y2) && map.checkStick(x1, y1, x2, y2)) {
                            int result = checkStickPlacement(map, playerNumber, x1, y1, x2, y2, depth, playerNumberOriginal, alpha, beta, maxMoveBorder);
                            if (result > maxMoveBorder) continue;
                            if (playerNumber == playerNumberOriginal) {
                            ...
                            if (result != WRONG && result > maxValue && result < maxMoveBorder)  {
                                maxValue = result;
                                move = new Move(x1, y1, x2, y2, result);
                            }

                            if (beta <= alpha && move != null) {
                ...
                if (otherPlayer.getPosition().getX() == newX && otherPlayer.getPosition().getY() == newY) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        ...
                        if (Stick.check(stickX, stickY) && Point.check(newX, newY)  && !map.getSticks().contains(new Stick(stickX, stickY))
                            ...
                            int result = checkMove(map, playerNumber, newX, newY, depth, playerNumberOriginal, alpha, beta, maxMoveBorder);
                            if (result > maxMoveBorder) continue;
                            if (playerNumber == playerNumberOriginal) {
                                alpha = Integer.max(alpha, result);
                            } else {
                    ...
                    if (playerNumber != playerNumberOriginal && result != WRONG && result < maxMoveBorder) result *= -1;
                    if (result != WRONG && result > maxValue) {
                        maxValue = result;
                        move = new Move(newX, newY, result);
                    }
